...
hist(hf1_2016$pf_score)
hist(hf1_2016$pf_expression_control)
```
...
# Scatterplot
attach(hf1_2016)
plot(pf_expression_control, pf_score, main="Scatterplot Pf_core against pf_expression_control",
   xlab="PF Score", ylab="Pf Expression Control", pch=19)

# Add a regression line
regression_model <- lm(pf_score ~ pf_expression_control)
...
```{r challenge}
hf1_2016 %>% select(pf_score, pf_expression_control) %>% cor()
```
```{r}
hf1_2016 %>% summarise(r = cor(pf_score, pf_expression_control))
```
